...
export default function KitchenDisplayPage() {
  ...
  useEffect(() => {
    ...
    socket.on('order_status_updated', (data) => {
      // In a real scenario, we'd append or update the local state efficiently
      fetchOrders();
    });
         ...
         <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
           {categories.map(cat => (
              <button 
                ...
                className={`h-14 px-6 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all whitespace-nowrap border ${
                ...
                {filteredOrders.filter(o => o.status === column.id).map(order => (
                  <div key={order._id} className="bg-white rounded-3xl border border-stone-100 shadow-card hover:shadow-xl transition-all duration-300 group overflow-hidden animate-fade-in relative">
                    ...
                    <div className={`absolute top-0 left-0 w-full h-1.5 ${
                      column.id === 'confirmed' ? 'bg-stone-200' : column.id === 'preparing' ? 'bg-amber-400' : 'bg-emerald-400'
                    }`} />
                      ...
                      <div className="flex items-center justify-between mb-6">
                        ...
                        <div className="text-right">
                           <div className="flex items-center gap-1.5 px-2.5 py-1 bg-stone-50 rounded-full border border-stone-100">
                             ...
                             <span className="text-[10px] font-black text-stone-700 tracking-tight">12m ago</span>
                           </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3 mb-8">
                        {order.lines.map((item) => (
                          <div 
                            ...
                            className={`flex items-start justify-between py-3 px-4 rounded-xl cursor-pointer transition-all active:scale-95 group/item border ${
                              item.isPrepared 
                          ...
                          >
                            <div className="flex items-start gap-3">
                               <div className={`mt-0.5 w-4 h-4 rounded border flex items-center justify-center transition-colors ${
                                 item.isPrepared ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-stone-300'
                               }`}>
